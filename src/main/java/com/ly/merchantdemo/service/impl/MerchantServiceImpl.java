...
public class MerchantServiceImpl implements MerchantService {
    ...
    @Override
    public MerchantResult<RequestPayMessageDTO> postForPlatformData(String testRequestData, HttpServletResponse response) throws Exception {
        String getUrl = "http://sitpay.e-nci.com/api/pay/gateway";
        ...
        MerchantResult<RequestPayMessageDTO> merchantResult = new ObjectMapper().readValue
                (queryHttpByConfigs(postUrlConfig), new TypeReference<MerchantResult<RequestPayMessageDTO>>() {
                });
        if (merchantResult.getCode() != MerchantResultEnum.EASY_PAY_SUCCESS.code()) {
            throw new RuntimeException(MerchantResultEnum.EASY_PAY_FAIL_GETURL.message());
        }
        merchantResult.setMessage(MerchantResultEnum.MERCHANT_SUCCESS.message());
        merchantResult.setCode(MerchantResultEnum.MERCHANT_SUCCESS.code());
    ...
    public String queryHttpByConfigs(HttpConfigs httpConfig) {
        ...
        Parameters params = new Parameters().appId(httpConfig.appId()).version(httpConfig.version()).signatureMethod(httpConfig.encryptType());
        ...
        return "post".equals(httpConfig.httpMethod())? resource.post(String.class, httpConfig.postQueryBody()):resource.get(String.class);
    }
}
